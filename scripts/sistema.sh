...
tarea_backup() {
    ...
    local cantidad
    cantidad=$(ls "$destino" 2>/dev/null | wc -l)
...
tarea_limpieza() {
    ...
    log "=== TAREA 2: Limpieza de archivos mayores a $DIAS_RETENTION dias ==="
    ...
    while IFS= read -r -d '' archivo; do
        ...
        find "$DIR_BACKUPS" -type f -mtime +"$DIAS_RETENTION" -print0 2>/dev/null
    )
...
main() {
    ...
    log "============================================"
    log "Iniciando script de automatizacion del sistema"
