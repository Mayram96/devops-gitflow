...
# ============================================
# verificar-permisos.sh - Auditoria de usuario
# TP02 - Operaciones1
# ============================================
...
ok() {
    echo "  [OK]   $1" | tee -a "$REPORTE"
}

fail() {
    echo "  [FAIL] $1" | tee -a "$REPORTE"
}

> "$REPORTE"

log "============================================"
log "  AUDITORIA DE PERMISOS - $(date)"
log "============================================"
log ""
...
if sudo -u "$USUARIO" bash -c \
    "echo test >> $APP_DIR/config/app.conf" 2>/dev/null; then
...
if sudo -u "$USUARIO" bash -c \
    "echo auditoria >> $APP_DIR/logs/audit.log" 2>/dev/null; then
...
log ""
log "============================================"
log "Auditoria completada: $REPORTE"
log "============================================"
